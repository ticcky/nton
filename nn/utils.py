...
def check_finite_differences(fwd_fn, bwd_fn, delta=1e-5, n_times=10, gen_input_fn=None):
    ...
    for n in range(n_times):
        ...
        assert grads[0].shape == rand_input[0].shape

        for dim, x in enumerate(rand_input[0].flat):
            orig = rand_input[0].flat[dim]
            rand_input[0].flat[dim] = orig + delta
            out1 = (fwd_fn(rand_input)[0] * dy[0]).sum()
            rand_input[0].flat[dim] = orig - delta
            out2 = (fwd_fn(rand_input)[0] * dy[0]).sum()
            rand_input[0].flat[dim] = orig

            grad_num = ((out1 - out2) / (2 * delta))
            grad_an = grads[0].flat[dim]
