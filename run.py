...
def plot(losses, eval_index, (train_wers, train_accs), (test_wers, test_accs),
         plot_filename):
...
def main(**kwargs):
  ...
  update_rule = nn.Adam(nton_inst.params, nton_inst.grads)
  ...
  for epoch in xrange(10000000):
    train_dialog = next(data_train)
    logging.info("Next dialog. First user utterance: %s", train_dialog[0][1][:20])

    nton_inst.grads.zero()

    (O, O_aux) = nton_inst.forward_dialog(train_dialog)

    # ((loss,), loss_aux) = SeqLoss.forward(
    #     (Y, np.array(list(x_a) + [-1] * (len(y) - len(x_a))),))
    # (dY,) = SeqLoss.backward(loss_aux, 1.0)

    # nton_inst.backward(aux, (dY, None))
