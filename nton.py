...
import nn
from nn import (LSTM, OneHotFromVocab, Sequential, LinearLayer, Softmax,
                Sigmoid, Vars,
...
class NTON(ParametrizedBlock):
  def __init__(self, n_cells, mgr_h_dims, n_db_keys, db_index, db_contents,
               db_mapping, vocab, index_vocab):
  ...
  def forward(self, dialog):
    ...
    tr_tm1 = tuple(
        np.ones((len(self.vocab),)) * 0.01 for _ in range(self.n_db_keys))
    ...
    for O_t, I_t in dialog_turns:
      ...
      ((O_hat_t,), O_hat_t_aux) = self.nlg.forward(
        (s_tm1, O_t_start, len(O_t) - 1,) + nlu_tm1 + tr_tm1 + db_t)
      ...
      # 4. Update tracker.
      (tr_t, tr_t_aux) = self.trackers.forward(
  ...
  def backward(self, aux, dO_hat):
    ...
    dnlu_tp1 = tuple(
        np.zeros((len(self.vocab),)) for _ in range(self.n_db_keys))
    ...
    for (sys,
         usr), dO_hat_t, dO_hat_t_aux, ddb_res_t_aux, ddb_count_t_aux, dnlu_t_aux, dtr_t_aux, ds_t_aux in items:
      n_steps += 1
      dh_t_lst = []
      d_st_lst = []
      ...
      (dI_t,) = self.nlu.backward(dnlu_t_aux,
                                  (sum(dh_t_lst),) + tuple(dnlu_t_lst))
