...
import nn
from nn import ParametrizedBlock, Vars, Sequential, LinearLayer, Softmax, LSTM, \
  DBMap
...
class NLG(ParametrizedBlock):
  ...
  def forward(self, inputs):
    ...
    dbg_state = []
    for i, (db_map_i, input) in enumerate(
        zip(nn.DEBUG.db_mapping, external_inputs)):
      dbg_state.append("%s=%s" % (
      nn.DEBUG.vocab.rev(db_map_i), nn.DEBUG.vocab.rev(np.argmax(input))))
    ...
    if y_in is not None:
      input_y = tuple(y_in) + (None,) * y_steps
    else:
      input_y = (None,) * y_steps

    for t, y_in_t in enumerate(input_y):
      ...
      ((h_t, c_t), h_t_aux) = self.lstm.forward(
          (y_in_t[np.newaxis, np.newaxis, :], h_tm1, c_tm1))
      h_t = h_t[0][0]
      c_t = c_t[0][0]
  ...
  def backward(self, aux, (dO, )):
    ...
    dc_tp1 = 0.0
    dy_in_t = 0.0
    for do_t, o_t_aux, o_star_t_aux, o_star_t_used, h_t_aux in reversed(
        zip(dO, aux['o_t'], aux['o_star_t'], aux['o_star_t_used'], aux['h_t'])):
